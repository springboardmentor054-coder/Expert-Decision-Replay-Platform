...
def create_decision(
    ...
    create_audit_log(
        ...
        decision_id=decision.id,
        action_type=AuditActionType.CREATE_DECISION,
...
def update_decision(
    ...
    create_audit_log(
        ...
        user_id=current_user.id,
        decision_id=decision.id,
        action_type=AuditActionType.UPDATE_DECISION,
...
def update_decision_status(
    ...
    notify_stakeholders(
        db,
        decision_id=decision.id,
        title=f"Decision Status Updated: {decision.title}",
        message=f"Status changed to {payload.status.value} by {current_user.full_name}. Reason: {summary}",
    ...
    create_audit_log(
        ...
        description=f"Changed decision '{decision.title}' status to {payload.status.value}",
    )
    return decision
...
def delete_decision(
    ...
    create_audit_log(
        ...
        user_id=current_user.id,
        decision_id=decision.id,
        action_type=AuditActionType.DELETE_DECISION,
...
def add_stakeholder(
    ...
    create_notification(
        ...
        title="Assigned to Decision",
        message=f"You have been added as a {payload.role.value} stakeholder to decision \"{decision.title}\" by {current_user.full_name}.",
        decision_id=decision.id
    )
